...
def parse_data(train_path, test_path):
    ...
    train_df.dropna(subset=['Report'], how='any', inplace=True)  # 只要Report这列中有缺失值，就把这行删掉
    ...
    test_y = []
    train_x.to_csv('{}/datasets/segfile/train_set.seg_x.txt'.format(BASE_DIR), index=None, header=False)
    train_y.to_csv('{}/datasets/segfile/train_set.seg_y.txt'.format(BASE_DIR), index=None, header=False)
    test_x.to_csv('{}/datasets/segfile/test_set.seg_x.txt'.format(BASE_DIR), index=None, header=False)


def preprocess_sentence(sentence):
    seg_list = segment(sentence.strip(), cut_type='word')  # 返回的是jieba.lcut(sentence)的结果
